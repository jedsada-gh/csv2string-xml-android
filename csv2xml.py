...
if "arg" in translations:
    is_have_arg = True
    index_arg = translations.index("arg")
    translations.remove("arg")

if "_android" in translations:
    is_have_arg = True
    index_arg = translations.index("_android")
    translations.remove("_android")

translations = list(filter(lambda x: len(x) == 2, translations))
    ...
    for i in range(0, count_loop):
        if "#" in row[0]:
            ...
            comment = etree.Comment(
                "=" * 15 + " " + comment_message + " " + "=" * 15)
            roots[i].append(comment)
        elif len(str(row[0])) > 1:
            string_result = row[i + 1].decode('utf-8')
            if len(row) > index_arg:
                result_arg = row[index_arg + 1].split("=")
            else:
                this.is_have_arg = False
            arg_traget = "{" + result_arg[0].decode('utf-8') + "}"
            if is_have_arg and arg_traget in string_result:
                string_result = string_result.replace(arg_traget,
                                                      result_arg[1])
